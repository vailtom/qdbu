...
When `dialogue` is present, you asked `dialogue.question_asked` about `dialogue.previous_request` and the user message is the ANSWER. Combine the original request with the answer and produce the expression. You may NOT ask again — this is not a conversation: with the information you now have, either return the expression or return it empty with a `reason`. If you must ask (rule 9), ask ONE question that covers everything missing at once, naming the candidate fields so the person only has to pick.
